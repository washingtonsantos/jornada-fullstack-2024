using Fina.Core.Requests.Transacoes;
using Fina.Core.Validators.Transacoes;
using FluentValidation.TestHelper;

namespace Fina.Api.Tests.Validators.Transacoes;

public class CriarTransacaoValidatorTests
{
    private readonly CriarTransacaoValidator _validator = new();

    [Fact]
    public void CriaTransacaoEfetivada_ReturnsSuccess()
    {
        var request = new CriarTransacaoRequest
        {
            Titulo = "Salario",
            Descricao = "Salario recebido por empresa XYZ",
            PagoOuRecebidoEm = DateTime.UtcNow,
            TipoTransacao = Core.Enums.TipoTransacao.Receita,
            Valor = 1000.00m,
            CategoriaId = Guid.Parse("D2AFC928-38E8-4D8B-8C40-000000000001"),
            SubCategoriaId = null,
            DataEfetivado = DateTime.UtcNow,
            FormaPagamentoRecebimento = Core.Enums.TipoPagamentoRecebimento.PIX,
            ContaId = Guid.Parse("bd7c019b-aa34-4447-9366-bc0e3366312d"),
            Efetivado = true,
            Recorrente = false,
            UsuarioId = Guid.Parse("DDF53685-7DFC-4593-BDA7-2FA5305C4283")
        };

        var result = _validator.TestValidate(request);
        result.ShouldNotHaveAnyValidationErrors();
    }

    [Fact]
    public void CriaTransacaoPendente_ReturnsSuccess()
    {
        var request = new CriarTransacaoRequest
        {
            Titulo = "Assinatura IPTV",
            Descricao = "Assinatura IPTV",
            PagoOuRecebidoEm = null,
            TipoTransacao = Core.Enums.TipoTransacao.Despesa,
            Valor = 1000.00m,
            CategoriaId = Guid.Parse("d2afc928-38e8-4d8b-8c40-000000000108"),
            FormaPagamentoRecebimento = null,
            ContaId = Guid.Parse("bd7c019b-aa34-4447-9366-bc0e3366312d"),
            Efetivado = false,
            Recorrente = false,
            UsuarioId = Guid.Parse("DDF53685-7DFC-4593-BDA7-2FA5305C4283")
        };

        var result = _validator.TestValidate(request);
        result.ShouldNotHaveAnyValidationErrors();
    }

    [Fact]
    public void CriaTransacaoAgendada_ReturnsSuccess()
    {
        var request = new CriarTransacaoRequest
        {
            Titulo = "Imposto de Renda DAS",
            Descricao = $"imposto de renda referente ao ano {DateTime.UtcNow.Year}",
            PagoOuRecebidoEm = DateTime.UtcNow.AddDays(10),
            TipoTransacao = Core.Enums.TipoTransacao.Despesa,
            Valor = 1000.00m,
            CategoriaId = Guid.Parse("d2afc928-38e8-4d8b-8c40-000000000108"),
            FormaPagamentoRecebimento = null,
            ContaId = Guid.Parse("bd7c019b-aa34-4447-9366-bc0e3366312d"),
            Efetivado = false,
            Recorrente = false,
            UsuarioId = Guid.Parse("DDF53685-7DFC-4593-BDA7-2FA5305C4283")
        };

        var result = _validator.TestValidate(request);
        result.ShouldNotHaveAnyValidationErrors();
    }

    [Fact]
    public void ComTituloEmBranco_ReturnsError()
    {
        var request = new CriarTransacaoRequest { Titulo = "" };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.Titulo);
    }

    [Fact]
    public void ComTituloCurto_ReturnsError()
    {
        var request = new CriarTransacaoRequest { Titulo = "AB" };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.Titulo);
    }

    [Fact]
    public void ComPagoOuRecebidoEmInvalido_ReturnsError()
    {
        var request = new CriarTransacaoRequest { PagoOuRecebidoEm = DateTime.MinValue };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.PagoOuRecebidoEm);
    }

    [Fact]
    public void ComTipoTransacaoInvalida_ReturnsError()
    {
        var request = new CriarTransacaoRequest { TipoTransacao = 0 };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.TipoTransacao);
    }

    [Theory]
    [InlineData(0)]
    [InlineData(-1)]
    public void ComValorInvalido_ReturnsError(decimal valor)
    {
        var request = new CriarTransacaoRequest { Valor = valor };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.Valor);
    }

    [Fact]
    public void ComCategoriaIdEmBranco_ReturnsError()
    {
        var request = new CriarTransacaoRequest { CategoriaId = Guid.Empty };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.CategoriaId);
    }

    [Fact]
    public void ComTipoPagamentoRecebimento_ReturnsError()
    {
        var request = new CriarTransacaoRequest { FormaPagamentoRecebimento = 0 };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.FormaPagamentoRecebimento);
    }

    [Fact]
    public void ComContaIdEmBranco_ReturnsError()
    {
        var request = new CriarTransacaoRequest { ContaId = Guid.Empty };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.ContaId);
    }

    [Fact]
    public void ComUsuarioIdEmBranco_ReturnsError()
    {
        var request = new CriarTransacaoRequest { UsuarioId = Guid.Empty };
        var result = _validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor(x => x.UsuarioId);
    }
}
